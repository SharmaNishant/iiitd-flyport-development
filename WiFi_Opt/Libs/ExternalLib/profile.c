...
void ProfileDefault()
{
	...
	profile.ScanPeriod = DEFAULT_SCANPERIOD;
	strcpy(profile.ApiKey, DEFAULT_APIKEY);
	strcpy(profile.UserKey,DEFAULT_USERKEY);
	...
	strcpy(profile.DjangoPort, DEFAULT_DJANGOPORT);
	strcpy(profile.ServerURL, DEFAULT_SERVERURL);
	profile.SNTPEnable = DEFAULT_SNTPENABLE;
	profile.AppEnable = DEFAULT_APPENABLE;
	profile.WifiEnable = DEFAULT_WIFIENABLE;
	profile.OffTime = DEFAULT_OFFTIME;
	profile.OnTime = DEFAULT_ONTIME;
	profile.OptMode = DEFAULT_OPTMODE;
	profile.OptLevel = DEFAULT_OPTLEVEL;
	profile.OnType = DEFAULT_ONTYPE;
