...
{	
    ...
    {
     			ptr = HTTPGetROMArg(curHTTP.data, (ROM BYTE *)"UserKey");   
				if(strcmp(ptr,profile.UserKey)!=0)
				{
					HTTP_UNAUTHORIZED;
					UARTWrite(1,"\r\nWrong key");
				}
...
void HTTPPrint_DJANGOPORT()
{
	TCPPutString(sktHTTP, (BYTE*)profile.DjangoPort);
	return;
}
...
void HTTPPrint_SCANPERIOD()
{
	char val[4];
	sprintf(val, "%d", profile.ScanPeriod);
	TCPPutString(sktHTTP, (BYTE*)val);
	return;
}
...
void HTTPPrint_USERKEY()
{
	TCPPutString(sktHTTP, (BYTE*)profile.UserKey);
	return;
}
...
void HTTPPrint_OFFTIME()
{
	char val[4];
	sprintf(val,"%d",profile.OffTime);
	TCPPutString(sktHTTP, (BYTE*)val);
	return;
}
	
void HTTPPrint_ONTIME()
{
	char val[4];
	sprintf(val,"%d",profile.OnTime);
	TCPPutString(sktHTTP, (BYTE*)val);
	return;
}
void HTTPPrint_SNTP()
...
void HTTPPrint_OPTLVL()
{
	char val[4];
	sprintf(val,"%d",profile.OptLevel);
	TCPPutString(sktHTTP, (BYTE*)val);
	return;
}

void HTTPPrint_OPTIMIZATION()
{
	char val[4];
	sprintf(val,"%d",profile.OptMode);
	TCPPutString(sktHTTP, (BYTE*)val);
	return;
}
void HTTPPrint_NETTYPE()
{
	char val[4];
	sprintf(val,"%d",profile.NetType);
	TCPPutString(sktHTTP, (BYTE*)val);
	return;
}
void HTTPPrint_ONTYPE()
{
	char val[4];
	sprintf(val,"%d",profile.OnType);
	TCPPutString(sktHTTP, (BYTE*)val);
	return;
}
void HTTPPrint_ENABLESCAN()
{
	char val[4];
	sprintf(val,"%d",profile.WifiEnable);
	TCPPutString(sktHTTP, (BYTE*)val);
	return;
}
...
HTTP_IO_RESULT HTTPExecutePost(void)
{
	...
	if(!memcmppgm2ram(filename, "protect/device.cgi",18)) 
	{
		...
		while(curHTTP.byteCount)
		{
			...
			else if(memcmppgm2ram(curHTTP.data,(ROM void*)"SCANPERIOD", 10) == 0)
			{
				memcpy(String_post,(void*)&curHTTP.data[11], len-11);
				String_post[len-11]='\0';
				profile.ScanPeriod = atoi(String_post);
			}
			...
			else if(memcmppgm2ram(curHTTP.data,(ROM void*)"DJANGOPORT", 10) == 0)
			{
				memcpy(String_post,(void*)&curHTTP.data[11], len-11);
				String_post[len-11]='\0';
				strcpy(profile.DjangoPort, String_post);
			}
			...
			else if(memcmppgm2ram(curHTTP.data,(ROM void*)"ENABLESCAN", 10) == 0)
			{
				memcpy(String_post,(void*)&curHTTP.data[11], len-11);
				String_post[len-11]='\0';
				profile.WifiEnable=1;
			}
		}
		ProfileSave();
	}
	
	if(!memcmppgm2ram(filename, "protect/optimization.cgi",24))
	{
		while(curHTTP.byteCount)
		{
			// Check for a complete variable
			...
			if(memcmppgm2ram(curHTTP.data,(ROM void*)"OPTIMIZATION", 12) == 0)
			{
				memcpy(String_post,(void*)&curHTTP.data[13], len-13);
				String_post[len-13]='\0';
				profile.OptMode=1;
			}
			else if(memcmppgm2ram(curHTTP.data,(ROM void*)"OPTLVL", 6) == 0)
			{
				memcpy(String_post,(void*)&curHTTP.data[7], len-7);	
				String_post[len-7] = '\0';
				if(String_post[0] == '1')
			...
			else if(memcmppgm2ram(curHTTP.data,(ROM void*)"ONTYPE", 6) == 0)
			{
				memcpy(String_post,(void*)&curHTTP.data[7], len-7);	
				String_post[len-7] = '\0';
				if(String_post[0] == '1')
			...
			else if(memcmppgm2ram(curHTTP.data,(ROM void*)"OFFTIME", 7) == 0)
			{
				memcpy(String_post,(void*)&curHTTP.data[8], len-8);	
				String_post[len-8] = '\0';
				profile.OffTime = atoi(String_post);
			}
			else if(memcmppgm2ram(curHTTP.data,(ROM void*)"ONTIME", 6) == 0)
			{
				memcpy(String_post,(void*)&curHTTP.data[7], len-7);	
				String_post[len-7] = '\0';
				profile.OnTime = atoi(String_post);
			}
		}
		ProfileSave();
	}
